...
class Create extends Component
{
    ...
    {
        return [
            'nomor_surat' => 'required|string|max:100|unique:sktm_documents,nomor_surat',
    ...
    {
        $this->validate();
        SktmDocument::create($this->except(['pejabats']) + [
            'kelurahan_id' => auth()->user()->kelurahan_id,
            'created_by' => auth()->id(),
            'status' => 'active',
        ]);
        session()->flash('message', 'SKTM berhasil dibuat.');
        $this->redirect(route('sktm.index'), navigate: true);
    }

    public function render()
    {
        return view('livewire.sktm.create');
    }
}
